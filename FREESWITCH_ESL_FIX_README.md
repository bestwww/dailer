...
3. **mod_enum отключен** - не критичен для Event Socket и автодозвона ✅
...
#### ✅ **Решение:**
- Добавлен флаг `--disable-mod-enum` в configure FreeSWITCH
- mod_enum используется для ENUM DNS запросов (не критично для Dailer)
...
- ✅ `--disable-mod-enum` - отключение ENUM модуля (не нужен)
...
- ✅ **T.38 факсы** (бонус)
- ❌ **ENUM DNS запросы** (отключены - не нужны для автодозвона)
