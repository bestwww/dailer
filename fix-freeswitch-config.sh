...
# 9. Тестовый звонок (правильный формат - без + и с 7)
log_info "Выполнение тестового звонка (формат: 79206054020)..."
...
if echo "$TEST_RESULT" | grep -q "INTERWORKING"; then
    log_warn "Ошибка INTERWORKING сохраняется"
    echo ""
    echo "🔍 ДОПОЛНИТЕЛЬНАЯ ДИАГНОСТИКА:"
    echo "==============================="
    echo "Попробуем другие варианты формата..."
    
    # Пробуем с +7
    log_info "Попытка с +7..."
    TEST_RESULT2=$(docker exec dialer_freeswitch_host fs_cli -x "originate sofia/gateway/sip_trunk/+79206054020 &echo" 2>&1)
    echo "Результат с +7: $TEST_RESULT2"
    
    echo ""
    echo "🔍 РЕКОМЕНДАЦИИ:"
    echo "================"
    echo "1. Формат номера должен быть: 79206054020 (начинается с 7)"
    echo "2. Caller ID правильный: 79058615815"
    echo "3. Возможно провайдер требует префикс или другие настройки"
    echo "4. Свяжитесь с провайдером для уточнения требований к номерам"
